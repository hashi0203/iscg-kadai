...
//const vec3 c_from = vec3(10,0,-2); // 水中カメラ
...
void main( void ) {
	...
	objects[7] = create_sphere(vec3(1,-6,-1),2.0,no_color,2,refr_ice); // 水面に浮かぶ氷の球
	objects[8] = create_sphere(vec3(2,3,-2.5),2.0,no_color,2,refr_glass); // 水中にあるガラスの球
	objects[9] = create_sphere(vec3(9.134,0,-4),1.0,no_color,2,refr_glass); // カメラが通るガラスの球
	objects[10] = create_cone(vec3(1,1,-0.5),vec3(0,2,4),3.0,4.0,vec3(0,1,0),0,no_refr); // 緑の円錐
	objects[11] = create_cone(vec3(-9,0,0),vec3(0,0,-1),22.0,30.0,no_color,2,refr_water); // 水中
	objects[12] = create_sphere(vec3(5,-4,-5),2.0,no_color,2,refr_air); // 水中の気泡
